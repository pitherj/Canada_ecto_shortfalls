...
This repo houses the reproducible analysis pipeline for the manuscript *An assessment of biodiversity data shortfalls for ectomycorrhizal fungi in Canada*. The manuscript and Supplemental Information documents themselves are prepared and archived separately (see Project structure below).
...
inputs. The manuscript (`manuscript.qmd`) and Supplemental Information
documents (`supplemental_information_S1.qmd`, `supplemental_information_S2.qmd`)
...
- **[Quarto](https://quarto.org)** — only needed if rendering the manuscript or
  Supplemental Information documents (prepared separately; not required to run
...
│   ├── 01_…_21_….R               # ordered pipeline (see scripts/README.md)
│   ├── run_all.R                 # master runner (sources 01–21 in order)
...
├── manuscript.qmd                # manuscript source — not tracked in this repo
└── supplemental_information_S1.qmd, _S2.qmd  # SI sources — not tracked in this repo
```

Note: `manuscript.qmd`, the two `supplemental_information_S*.qmd` files, their
rendered outputs (`.docx`/`.html`/`.pdf`), `references.bib`, and
`harvard-cite-them-right.csl` live in this project folder locally but are
excluded from the GitHub repo via `.gitignore` — they're prepared and archived
...
|---|---|---|
| Figure 1 | `figures/Figure-01_sampling_map.png` (+ grey variant, see below) | `20_sampling_maps.R` |
| Figure 2 | `figures/Figure-02_wallacean_occupancy.png` | `11_wallacean.R` |
| Figure 3 | `figures/Figure-03_climate_gap.png` (+ grey variant, see below) | `18_climate_gap.R` |
| Figure 4 | `figures/Figure-04_host_bivariate_map.png` (+ grey variant, see below) | `17_hutchinsonian.R` |
...
| Figure S3 | `figures/Figure-S3_depth_discard.png` | `21_depth_discard.R` |
| Figure S4 | `figures/Figure-S4_ecozone_sampling_map.png` (+ grey variant, see below) | `17_hutchinsonian.R` |
| Figure S5 | `figures/Figure-S5_gbif_specimens.png` (+ grey variant, see below) | `20_sampling_maps.R` |
| Tables 1–4, S1–S3 | `data_derived/` CSVs, rendered into tables by `supplemental_information_S1.qmd`/`S2.qmd` (prepared separately) | 09, 11, 16, 17, 19 |
...
`18`, and `20` each save a second, `_grey`-suffixed copy of their figure(s) on
...
`figures/`; only the white versions are referenced by `manuscript.qmd` / the
SI documents.
...
| `manuscript.qmd` | Manuscript source (not tracked in this repo — see Project structure) |
| `supplemental_information_S1.qmd` | Supplemental Methods (not tracked in this repo) |
| `supplemental_information_S2.qmd` | Supplemental tables/figures + decision log (not tracked in this repo) |
...
References section of the manuscript/Supplemental Information (prepared

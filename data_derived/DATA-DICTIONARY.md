...
├── globalfungi_sample_type_tally.csv   # (02)
...
├── geo_possibility/              # Eltonian denominator sensitivity check (21)
```
...
| `globalfungi_sample_type_tally.csv` | `02_globalfungi.R` | Sample-type composition of GlobalFungi v5, tallied from the raw metadata file at two scopes: worldwide and Canada (country filter only, so the two are comparable). Read by the manuscript Methods and by SM1 so the composition percentages are computed rather than typed. |
...
### `globalfungi_sample_type_tally.csv` — fields

| Field | Type | Description |
|---|---|---|
| `scope` | character | `worldwide` (every sample in the raw metadata file) or `canada` (samples whose `country` is Canada). The Canada rows apply a country filter only — deliberately not the fuller Canadian selection used to build the analysis dataset — so the two scopes describe the database on the same terms. |
| `sample_type` | character | GlobalFungi `sample_type` value (soil, root, shoot, topsoil, rhizosphere soil, …). |
| `n_samples` | integer | Distinct samples of that type within that scope. |
| `pct` | numeric | Percentage of that scope's samples, to one decimal place. |
...
| `eltonian_host_matching.csv` | Host-name matching results for the ANALYSIS set: raw input, cleaned name, match status against the native host list, restricted to root-derived evidence (GlobalFungi root samples; GenBank records whose isolation source names root or ectomycorrhizal material). Every reported fungus–host association derives from this file. |
| `eltonian_host_names_metadata.csv` | The same columns without the root-evidence restriction, plus a `root_evidence` flag: every host name the metadata carries, whatever the tissue. Documents host-name quality rather than interaction evidence, and **feeds Tables S2 and S3.** |
...
| `eltonian_sample_type_tally_canada.csv`, `eltonian_sample_type_tally_global.csv`, `eltonian_genbank_tissue_tally_canada.csv` | Diagnostic tallies of the sample/tissue types underlying the host information. The GenBank tally also carries three roll-up rows (root-derived, tissue provenance unknown, demonstrably non-root) matching how the manuscript reports them. |
| `eltonian_sample_type_tally_canada_all.csv` | Sample-type composition of **all** EcM-positive Canadian GlobalFungi samples, with no dominant-plant-label restriction. Companion to the worldwide/Canada tally in `globalfungi_sample_type_tally.csv`. |
| `eltonian_pair_confidence.csv` | How many observed host × fungus pairs rest on records naming a single candidate plant ("unambiguous") versus several ("ambiguous"), for the SH-code and named-species pair sets. |
| `eltonian_sensitivity_confidence.csv` | The headline Canadian statistics (hosts documented, shortfall, fungal species documented, pairs) recomputed from unambiguous evidence only, as a sensitivity check on the pair set. |
...
| File | Field | Type | Description |
|---|---|---|---|
| `eltonian_host_matching.csv`, `eltonian_host_names_metadata.csv` | `sh_code` | character | UNITE SH code of the fungal record |
| | `genus`, `species` | character | Fungal taxonomy |
| | `host_raw` | character | Raw host string as recorded (may name several plants) |
| | `host_field` | character | Source metadata field the host string came from |
| | `host_item` | character | One plant name cut out of `host_raw` |
| | `host_rank` | integer | Position of `host_item` within its field's label (1 = listed first) |
| | `n_hosts_in_field` | integer | How many plants that one field named |
| | `n_hosts_in_record` | integer | How many candidate plants the whole source record named (both GlobalFungi plant fields together; the GenBank host field alone) |
| | `pair_confidence` | character | "unambiguous" if `n_hosts_in_record` is 1, otherwise "ambiguous" |
| | `root_evidence` | logical | *(`eltonian_host_names_metadata.csv` only)* Whether the record is root-derived and so reaches the association analyses |
| | `host_clean` | character | Cleaned host name (from `host_item`) |
...
| | `n_records_unambiguous` | integer | How many of those records named a single candidate plant |
| | `pair_confidence` | character | "unambiguous" if any supporting record did, otherwise "ambiguous" |
| `eltonian_pair_confidence.csv` | `pair_set` | character | "host x SH code" or "host x named fungal species" |
| | `pair_confidence` | character | "unambiguous" or "ambiguous" |
| | `n_pairs`, `pct` | integer, numeric | Pair count and its percentage within that pair set |
| `eltonian_sensitivity_confidence.csv` | `metric`, `value` | character, numeric | Headline statistic and its value under all evidence vs unambiguous evidence only |
...
| `eltonian_sample_type_tally_canada_all.csv` | `sample_type` | character | GlobalFungi `sample_type` value |
| | `n_samples`, `pct` | integer, numeric | Sample count and its percentage of all EcM-positive Canadian samples |
...
## `geo_possibility/` — geographic-possibility sensitivity check

Produced by `21_geographic_possibility.R`. Asks how much of the Eltonian host ×
fungal-species grid is geographically possible at all, at six spatial grains,
as a sensitivity check on the reported denominator. The manuscript keeps the
full grid (framed as a metaweb) and cites these outputs only to show that the
conclusion does not depend on that choice; the reasons the filtered denominator
is not adopted are given in SM1 and in the script header.

| File | Description |
|---|---|
| `output1_possible_pairs_grain_a.csv` | The possibility matrix in long form at the finest grain (point inside the host's modelled range). **Sparse:** only cells with `possible == 1` are listed; any pair absent from the file is impossible at that grain. |
| `output2_grain_sensitivity.csv` | One row per spatial grain: number and proportion of grid cells that are geographically possible, plus mean/median possible hosts per fungus. |
| `output3_effort_by_species.csv` | One row per fungal species with a georeferenced record: occurrence-record count, distinct-location count, and possible-host-set size. |
| `output3_rarefaction.csv` | Rarefaction curves of possible-host-set size at *k* = 1, 2, 3, 5, 10, 20 records, for two species sets; carries the random seed and number of replicate draws. |
| `output3_effort_limitation.png` | Two-panel diagnostic figure: possible-host-set size vs record count, and the rarefaction curves. |
| `output4_observed_but_impossible.csv` | Observed host × fungus pairs flagged geographically impossible at the finest grain, with a `reason` column separating range/coordinate failure from untestable pairs. |
| `output5_fill_statistics.csv` | The Eltonian fill statistics recomputed against the geographically-possible denominator at each grain, alongside the full-matrix values. |
| `output6_summary.csv` | The headline numbers as a long-format `metric,value` table; read directly by the manuscript and SM1. |
| `run_log.txt` | Console record of the run, including the random seed and the denominator check against `eltonian_summary.csv`. |

#### `geo_possibility/` fields

| File | Field | Type | Description |
|---|---|---|---|
| `output1_possible_pairs_grain_a.csv` | `host_species`, `fungal_species` | character | The pair |
| | `possible` | integer (1) | Always 1; the file is sparse |
| | `n_records_supporting` | integer | Georeferenced Canadian records of that fungus falling inside that host's range |
| `output2_grain_sensitivity.csv` | `grain`, `grain_description` | character | Spatial grain identifier and its plain description |
| | `n_matrix_cells` | integer | Cells in the full grid (constant across rows) |
| | `n_cells_possible`, `prop_cells_possible`, `pct_cells_possible` | integer, numeric | Cells possible at that grain, as a count, proportion, and percentage |
| | `mean_possible_hosts_per_fungus`, `median_possible_hosts_per_georef_fungus` | numeric | Possible-host-set size per fungal species |
| `output3_effort_by_species.csv` | `fungal_species` | character | Named EcM fungal species |
| | `n_records`, `n_locations` | integer | Georeferenced Canadian records, and distinct coordinates |
| | `n_possible_hosts` | integer | Possible-host-set size at the finest grain |
| `output3_rarefaction.csv` | `species_set` | character | `all_eligible` or `n_ge_20` (the constant-membership saturation test) |
| | `k`, `n_species` | integer | Records subsampled per species, and how many species contributed |
| | `mean_hosts`, `median_hosts`, `q25_hosts`, `q75_hosts` | numeric | Possible-host-set size across replicate draws |
| | `seed`, `n_reps` | integer | Random seed and number of draws, stored for reproducibility |
| `output4_observed_but_impossible.csv` | `host_species`, `fungal_species` | character | The observed pair |
| | `n_occurrences`, `n_records_unambiguous`, `pair_confidence` | integer, integer, character | Carried through from `eltonian_species_occurrence_counts.csv` |
| | `geo_possible`, `host_has_range`, `fungus_has_georef` | logical | Test outcome and whether each side was testable |
| | `reason` | character | `range_or_coord_error`, `fungus_no_georef`, or `no_bien_range` |
| `output5_fill_statistics.csv` | `grain`, `grain_description` | character | Spatial grain (including a `full_matrix` baseline row) |
| | `n_denominator_cells` | integer | Cells in that denominator |
| | `n_filled_all`, `pct_filled_all`, `pct_empty_all` | integer, numeric | Using all observed pairs — directly comparable to the manuscript |
| | `n_filled_possible_only`, `pct_filled_possible_only`, `pct_empty_possible_only` | integer, numeric | Using only observed pairs that are also possible at that grain |
| | `n_singleton_all`, `pct_singleton_of_observed_all`, `pct_singleton_of_denominator`, `n_singleton_possible_only`, `pct_singleton_of_observed_possible_only` | integer, numeric | Single-record pair counts and shares, on both bases |
| `output6_summary.csv` | `metric`, `value` | character, numeric | Headline statistic and its value |
